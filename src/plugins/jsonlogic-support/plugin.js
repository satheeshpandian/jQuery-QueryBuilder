...
QueryBuilder.defaults({
    jsonLogicOperators: {
        // @formatter:off
        equal:            function(f, v) { return v[0]; },
        not_equal:        function(f, v) { return { '$ne': v[0] }; },
        in:               function(f, v) { return { '$in': v }; },
        not_in:           function(f, v) { return { '$nin': v }; },
        less:             { symbol: '<', func: function(f, v) { return [{'var': f}, v[0]]; } } ,
        less_or_equal:    function(f, v) { return { '$lte': v[0] }; },
        greater:          { symbol: '>', func: function(f, v) { return [{'var': f}, v[0]]; } },
        greater_or_equal: function(f, v) { return { '$gte': v[0] }; },
        between:          function(f, v) { return { '$gte': v[0], '$lte': v[1] }; },
        not_between:      function(f, v) { return { '$lt': v[0], '$gt': v[1] }; },
        begins_with:      function(f, v) { return { '$regex': '^' + Utils.escapeRegExp(v[0]) }; },
        not_begins_with:  function(f, v) { return { '$regex': '^(?!' + Utils.escapeRegExp(v[0]) + ')' }; },
        contains:         function(f, v) { return { '$regex': Utils.escapeRegExp(v[0]) }; },
        not_contains:     function(f, v) { return { '$regex': '^((?!' + Utils.escapeRegExp(v[0]) + ').)*$', '$options': 's' }; },
        ends_with:        function(f, v) { return { '$regex': Utils.escapeRegExp(v[0]) + '$' }; },
        not_ends_with:    function(f, v) { return { '$regex': '(?<!' + Utils.escapeRegExp(v[0]) + ')$' }; },
        is_empty:         function(f, v) { return ''; },
        is_not_empty:     function(f, v) { return { '$ne': '' }; },
        is_null:          function(f, v) { return null; },
        is_not_null:      function(f, v) { return { '$ne': null }; }
        // @formatter:on
    },
